 ...
 *
 * Wang Bingxuan 120L022115 2003007
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
int main(int argc, char** argv)
{
	...
	while ((c = getopt(argc, argv, "hvp")) != EOF && c!=0xff) {
		switch (c) {
...
int builtin_cmd(char** argv)
{
	...
	if (strcmp("quit", head) == 0) {//quit
		//printf("Quit Shell! Back Control Terminate!\nGoodBye!");
		exit(0);
	}
		...
		printf("    PID JID STATE CMDLINE\n");
		char STATE[4][5] = { "UNDEF","FG","BG","ST" };
		...
		for (i = 0; i < MAXJOBS; i++) {
			if (jobs[i].state != UNDEF)
				printf("%7d %3d %5s %s", jobs[i].pid, jobs[i].jid, STATE[jobs[i].state], jobs[i].cmdline);
		}
		return 1;*/
	}
...
void waitfg(pid_t pid)
{
	...
	for (i = 0; i < MAXJOBS; i++) {
		if (jobs[i].pid == pid) {
			while (jobs[i].pid == pid && jobs[i].state == FG) {
				sleep(1);
				//printf("WAITFG: %d\n", jobs[i].pid);
			}
			//printf("RETURN %d\n", jobs[i].pid);
